...
function main(;
    ...
    (; ipsi, Ca, Fcc, regionPero, iphia, T, za, Na, Ca) = parameter_set()
    ...
    Ea1 = data1.params.bandEdgeEnergy[iphia, :]
    Ea2 = data2.params.bandEdgeEnergy[iphia, :]
    Ea3 = data3.params.bandEdgeEnergy[iphia, :]
    ################################################################################
    ################################################################################
    ...
    if printText
        ...
        Int = integrated_density(ctsys1, sol = sol1, icc = iphia, ireg = regionPero)
        mOmega = data1.regionVolumes[regionPero]

        println("Avg na for planar is:                     $(Int / mOmega)")

        ####################################################

        Int = integrated_density(ctsys2, sol = sol2, icc = iphia, ireg = regionPero)
        mOmega = data2.regionVolumes[regionPero]

        println("Avg na for textured ($textampl nm) is:       $(Int / mOmega)")

        #####################################################

        Int = integrated_density(ctsys3, sol = sol3, icc = iphia, ireg = regionPero)
        mOmega = data3.regionVolumes[regionPero]

        println("Avg na for textured ($textampl2 nm) is:       $(Int / mOmega)")
    ...
    if saveFig
        savefig(datadir("2D-planar-na-generation-Maxwell-forw-$V.pdf"))
    end
    ...
    if saveFig
        savefig(datadir("2D-nanotextured-ampl-$textampl-na-generation-Maxwell-forw-$V.pdf"))
    end
    ...
    if saveFig
        savefig(datadir("2D-nanotextured-ampl-$textampl2-na-generation-Maxwell-forw-$V.pdf"))
    end
