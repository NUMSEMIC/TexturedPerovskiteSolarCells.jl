...
Code for visualizing electron and hole density

=#

module FigS6ElectronAndHoleDensity
...
function main(;
        ...
        vHTL = 10, # 500   # in cm/s
        V = "inival", # "V-1p2",
        plotElectrons = true, plotHoles = true,
    ...
    (; ipsi, Ca, iphin, iphip, Fcc, regionPero, T, zn, zp, Nn, Np, En, Ep, Ca) = parameter_set()
    ...
    coord1 = subg1[Coordinates]; coord1 = coord1 ./ nm
    coord2 = subg2[Coordinates]; coord2 = coord2 ./ nm
    coord3 = subg3[Coordinates]; coord3 = coord3 ./ nm
    ...
    if plotElectrons
        ...
        if saveFig
            savefig(datadir("2D-planar-nn-generation-Maxwell-forw-$V.pdf"))
        end
        ...
        if saveFig
            savefig(datadir("2D-nanotextured-ampl-$textampl-nn-generation-Maxwell-forw-$V.pdf"))
        end
        ...
        if saveFig
            savefig(datadir("2D-nanotextured-ampl-$textampl2-nn-generation-Maxwell-forw-$V.pdf"))
        end
    ...
    if plotHoles
        ...
        if saveFig
            savefig(datadir("2D-planar-np-generation-Maxwell-forw-$V.pdf"))
        end
        ...
        if saveFig
            savefig(datadir("2D-nanotextured-ampl-$textampl-np-generation-Maxwell-forw-$V.pdf"))
        end
        ...
        if saveFig
            savefig(datadir("2D-nanotextured-ampl-$textampl2-np-generation-Maxwell-forw-$V.pdf"))
        end
    ...
    ###################################################################
    ni = sqrt(Nn[regionPero] * Np[regionPero] * exp(- (En[regionPero] - Ep[regionPero]) / (k_B * T)))
    ###################################################################

    mOmega = data1.regionVolumes[regionPero]

    nnAvg1 = integrated_density(ctsys1, sol = sol1, icc = iphin, ireg = regionPero) / mOmega
    npAvg1 = integrated_density(ctsys1, sol = sol1, icc = iphip, ireg = regionPero) / mOmega

    IntProd1 = ChargeTransport.integrate(ctsys1, DensityProduct, sol1)[1, regionPero] / mOmega

    VOC1 = k_B * T * log(IntProd1 / ni^2) / q
    #####################################################

    ## calculate the measure of the perovskite region
    mOmega = data2.regionVolumes[regionPero]

    nnAvg2 = integrated_density(ctsys2, sol = sol2, icc = iphin, ireg = regionPero) / mOmega
    npAvg2 = integrated_density(ctsys2, sol = sol2, icc = iphip, ireg = regionPero) / mOmega

    IntProd2 = ChargeTransport.integrate(ctsys2, DensityProduct, sol2)[1, regionPero] / mOmega

    VOC2 = k_B * T * log(IntProd2 / ni^2) / q

    #####################################################

    mOmega = data3.regionVolumes[regionPero]

    nnAvg3 = integrated_density(ctsys3, sol = sol3, icc = iphin, ireg = regionPero) / mOmega
    npAvg3 = integrated_density(ctsys3, sol = sol3, icc = iphip, ireg = regionPero) / mOmega

    IntProd3 = ChargeTransport.integrate(ctsys3, DensityProduct, sol3)[1, regionPero] / mOmega

    VOC3 = k_B * T * log(IntProd3 / ni^2) / q
    ...
    if printText
        ...
        println("Expected VOC for planar is:               $(VOC1)")
        println("Expected VOC for textured ($textampl m) is:  $(VOC2)")
        println("Expected VOC for textured ($textampl2 m) is:  $(VOC3)")
