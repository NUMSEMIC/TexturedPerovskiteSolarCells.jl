...
function main(;
    ...
    (; iphip, regionPero, heightDev) = parameter_set()

    @local_unitfactors nm cm
    ...
    G1 = zeros(num_nodes(subg1)); G2 = zeros(num_nodes(subg2)); G3 = zeros(num_nodes(subg3))

    ## photogeneration
    G1 .= view(data1.generationData[:], subg1)
    G2 .= view(data2.generationData[:], subg2)
    G3 .= view(data3.generationData[:], subg3)
    ...
    if printText
        ...
        println("Photogen integral for planar is:              $(round(IntG[iphip, regionPero] .* (cm^2) .* 1.0e3 ./ heightDev, digits = 3)) mA/cm^2.")

        IntG = ChargeTransport.integrate(ctsys2, Photogeneration!, sol2)
        println("Photogen integral for textured ($textampl m) is: $(round(IntG[iphip, regionPero] .* (cm^2) .* 1.0e3 ./ heightDev, digits = 3)) mA/cm^2.")

        IntG = ChargeTransport.integrate(ctsys3, Photogeneration!, sol3)
        println("Photogen integral for textured ($textampl2 m) is: $(round(IntG[iphip, regionPero] .* (cm^2) .* 1.0e3 ./ heightDev, digits = 3)) mA/cm^2.")
